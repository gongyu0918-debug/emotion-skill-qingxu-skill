...
| “路径对不上” | 先对齐目标，解释下一步，最多问一个澄清问题 |
| “已经好了，收口” | 进入 guard mode，做回归检查，防止继续漂移 |

默认 host 输出保持克制：内部状态会被翻译成正向行动提示，避免把 raw affect vector 直接塞给模型。
...
- `guidance.tone`：紧凑语气目标，比如 `evidence_first` 或 `careful_and_bounded`。
...
Raw affect 字段默认留在内部。审计和校准时显式开启：
...
开启后会增加 `diagnostics.internal.labels` 和 `diagnostics.internal.emotion_vector`，默认 host state 仍保持干净。
