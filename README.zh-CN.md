...
它会把“还没修好”“先给依据”“别碰配置”“现在可以了，开始收口”这类信号，转成线程优先级、验证强度、回复风格、进度更新节奏、成功后守护策略和紧凑路由原因。
...
- `route_reasons`：记录本轮为什么改变路由，避免暴露原始用户文本。
- `response_constraints`：把“先给依据、先验证、收口防漂移”这类约束直接交给 Agent。
- `satisfaction_lock`：用户已经满意时，把行为锁到收口和回归检查。
- `state.state_delta`：对比上一轮宿主持有的状态，识别信任下降、烦躁上升等趋势。
...
新增的 host 控制字段都是派生码和小型数值变化。旧宿主可以直接忽略，新宿主可以用它们做静默体验优化：

- 路由日志：`route_reasons`
- 回复约束：`response_constraints`
- 成功后防漂移：`satisfaction_lock`
- 跨轮趋势：`state.state_delta`
...
- `state.state_delta`：相对 `last_state` 的显著跨轮变化。
- `labels`：这一轮同时存在的状态。
- `mode`：这一轮真正驱动路由的主状态。
